classdef Filter_Adaptive_Histogram_Rayleigh < AbstractFilter
    %FILTER_CONTRASTTRANSFORM Filter that does contrast transformations
    
    properties
        Name = 'Adaptive Rayleigh Histogram Transformation';
    end
    
    methods
        function obj = Filter_Adaptive_Histogram_Rayleigh()
            % Create settings:
        ...
        function img_out = process(obj,img_in,settingValues)
            ...
            img_out = adapthisteq(img_in,'NumTiles',[8 8],'ClipLimit',c,'NBins',b,'Distribution','rayleigh','Alpha',d);
        end
    end
end